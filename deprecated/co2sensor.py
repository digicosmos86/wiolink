class CO2Sensor(GroveI2CDevice):
    ...
    def __check_refresh(self):
        now = ticks_ms()
        if self.last_measure == 0 or now - self.last_measure > self.get_measurement_interval():
            self.measures = self.read_measurement()
    ...
    def send_data(self, screen, line, line2=None, text1=None, text2=None):
        ...
        if text1 is None:
            msg1 = ">{0}: {1} ppm".format(self.port, reading[0])
        else:
            msg1 = "{0}: {1} ppm".format(text1, reading[0])
        if text2 is None:
            msg2 = ">{0}: {1}F {2}%".format(self.port, reading[1], reading[2])
        else:
            msg2 = "{0}: {1}F {2}%".format(text2, reading[1], reading[2])
